...
**Canonical reference for PAI skill organization, routing patterns, ecosystem, and management**
...
In PAI's architecture, **skills are the primary organizational primitive** - self-contained packages of context, workflows, and capabilities that enable specialized functionality.
...
- PAI activates `research` skill → Loads context from research/SKILL.md
...
- PAI reads workflow file and executes steps
...
Based on analysis of 29 production skills in PAI, three clear organizational patterns emerged:
...
3. **Directory Scanning**
   ...
   ```
   → PAI scans directory and presents options
...
4. **Paired Documentation**
   ...
   ```
   → PAI reads .md for instructions, executes .ts for automation
...
Expected: PAI activates skill-name, loads SKILL.md
```
...
**Quick Reference: All PAI Skills**

This section provides a comprehensive directory of all skills in the PAI ecosystem, organized by category with capabilities and usage guidance.

### Overview

PAI is organized around **topic-based skills** following the Skills-as-Containers architecture. Each skill is a self-contained domain with workflows, documentation, and references organized using **progressive disclosure** - load only what you need, when you need it.
...
**Domain:** PAI identity, contacts, preferences, security, architecture
**Key Capabilities:**
- User's complete context (contacts, preferences, voice IDs)
...
**When to use:** Automatically loaded - provides foundation for all PAI operations
**Location:** `~/.claude/skills/CORE/`

---

**system**
Complete PAI system infrastructure and operational tooling.
...
- **Voice & Style** - Direct, conversational, User's authentic voice with custom components
...
**personal**
User's personal context and knowledge management.
...
**projects**
Overview of User's active projects.
...
- **Anthropic Tracking** - Monitor product changes, actionable recommendations for PAI
...
```
User request → Main Agent (PAI) analyzes intent
             ↓
...
**Skills:** PAI-specific domain knowledge
...
Complete framework for creating new skills or updating existing ones, following Anthropic standards and PAI-specific patterns.
...
- Self-contained but inherits PAI context
...
2. **Relevance to User's Needs**
   - Matches PAI technology stack (TypeScript, bun, modern tools)
...
[Skills that solve current PAI gaps or enhance existing workflows]
...
3. Adapt to PAI patterns (TypeScript, structured format, etc.)
...
Based on User's PAI needs, prioritize these categories:
...
- No duplication of existing PAI capabilities
...
3. **Adapt** - Modify for PAI patterns if needed
...
1. **Creation**: Follow Anthropic standards + PAI patterns
2. **Discovery**: Multi-source research with quality filtering
3. **Personalization**: Align with User's stack and needs
4. **Quality**: Prioritize well-maintained, documented skills
5. **Integration**: Ensure PAI compatibility
...
- `~/.claude/skills/CORE/CONSTITUTION.md` - Overall PAI architecture and philosophy
