...
### 4. **Stop**
**When:** Main agent (PAI) completes a response
**Use Cases:**
...
- `stop-hook.ts` - THE CRITICAL HOOK for main agent completions
  ...
  - Sends to voice server with PAI's voice ID (`your-main-voice-id`)
...
**Key Variables:**
- `PAI_DIR` - PAI installation directory (always `/Users/username/.claude`)
- `DA` - Digital Assistant name ("PAI")
...
const completionMessage = extractPAICompletion(lastMessage);

const payload = {
  title: 'PAI',
  message: completionMessage,
  voice_enabled: true,
  voice_id: 'your-main-voice-id'  // PAI's ElevenLabs voice
};
...
- Main agent (PAI): `your-main-voice-id`
- Engineer: `your-engineer-voice-id`
...
curl -X POST http://localhost:8888/notify \
  ...
  -d '{"message":"Test message","voice_id":"your-main-voice-id","title":"Test"}'
```
...
Main Voice: your-main-voice-id (PAI)
...
**Maintainer:** PAI Maintainer (user@example.com)
