/**
 * Index formatting and load functionality for ctx CLI
 * 
 * Provides:
...
export function formatIndexTable(
  ...
  if (tagMatches.length > 0 || semanticMatches.length > 0) {
    lines.push("─".repeat(80));
    lines.push("Load options: ctx load <selection>");
    lines.push("  • ctx load all              - Load all results");
    lines.push("  • ctx load 1,2,5            - Load specific items");
    lines.push("  • ctx load 1-5              - Load range");
    lines.push("  • ctx load --type transcript - Load by type");
    lines.push("  • ctx load --since 2025-12-08 - Load by date");
  }
...
export async function loadBySelection(
  ...
  if (!index) {
    throw new Error("No previous search found. Run 'ctx search' or 'ctx semantic' first.");
  }
