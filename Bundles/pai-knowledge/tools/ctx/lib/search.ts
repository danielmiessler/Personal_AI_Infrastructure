/**
 * Search functionality for ctx CLI
...
import { findNotesByTagsAnd, isTagIndexAvailable } from "./tag-index";
...
export interface SearchOptions {
  ...
  noIndex?: boolean;    // Disable tag index (for benchmarking)
}
...
export async function searchNotes(options: SearchOptions): Promise<SearchResult[]> {
  ...
  // Check if we can use tag index for tag-only searches
  const useTagIndex = !options.noIndex &&
                      options.tags.length > 0 &&
                      !options.text &&
                      !options.untagged &&
                      await isTagIndexAvailable();

  if (useTagIndex) {
    // Use indexed tag lookup (fast path)
    const indexedNotes = await findNotesByTagsAnd(options.tags);

    // Convert to SearchResult format
    for (const note of indexedNotes) {
      try {
        const parsed = await parseNote(note.path);
        const stats = await stat(note.path);
        results.push({
          name: note.name,
          path: note.path,
          tags: note.tags,
          frontmatterTags: parsed.frontmatterTags,
          date: parsed.date,
          captureDate: parseDateString(parsed.date),
          mtime: stats.mtime,
    ...
    // Filter by tags if specified (only needed for non-indexed path)
    if (options.tags.length > 0 && !useTagIndex) {
