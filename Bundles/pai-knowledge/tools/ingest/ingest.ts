...
import { processMessage, saveToVault, cleanupDropFolderFile, type SaveResult } from "./lib/process";
import {
  initDb,
  getLastOffset,
  setLastOffset,
...
DIRECT INGEST OPTIONS (via Telegram - same as iOS shortcuts):
  ...
  --pipeline <name>      Force pipeline:
                           ...
                           /file    - rename with TYPE-DATE-TITLE + Dropbox sync
                           /archive - (deprecated, use /file)
...
EXAMPLES:
  ...
  ingest search --person john_smith       # Find mentions of John
    ...
    switch (command) {
      case "poll":
        if (commandArgs.includes("--help") || commandArgs.includes("-h")) {
          console.log(`ingest poll - Preview pending Telegram messages

USAGE:
  ingest poll [--verbose]

DESCRIPTION:
  Polls Telegram for new messages and shows a preview.
  This is READ-ONLY - it does not process or advance the offset.
  Use 'ingest process' to actually process messages.

OPTIONS:
  --verbose, -v   Show additional details (offset, etc.)

EXAMPLES:
  ingest poll              # Preview pending messages
  ingest poll --verbose    # With debug info
`);
          break;
        }
      ...
      case "direct":
        ...
        break;
      case "file":
        await handleFile(verbose, commandArgs);
        break;
      case "status":
        await handleStatus();
        break;
...
1. PARA Classification
   #para/project   - Has deadline, finite
   #para/area      - Ongoing responsibility
   #para/resource  - Reference material
   #para/archive   - Cold storage
   Shortcuts: #project #area #resource #archive

2. Project/Area Instance (use with PARA tag)
   #project/pai  #project/website
   #lifeos/knowledge  #lifeos/health

3. Note Type (Zettelkasten)
   #type/fleeting    - Raw capture (24-48h)
   #type/literature  - From external source
   #type/permanent   - Your atomic ideas
   #type/synthesized - Amalgamated

4. Status (Lifecycle)
   #status/inbox      - Needs processing
   #status/cultivated - Fully processed
   #status/archived   - Cold storage

5. Source
   #source/telegram  #source/voice  #source/web  #source/cli

6. Scope
   #scope/work  #scope/private
   Shortcuts: ~work  ~private

7. Topic
   #topic/ai  #topic/pkm  (free-form)
...
9. Format
   #format/transcript  #format/meeting-notes

10. Session
    #session/research-2025-12-20

11. Git
    #git/track - Include in version control
`;

const CLI_HELP_PARA = `
📁  PARA METHOD

"Organize by what you can DO with information, not by topic."

TWO TAGS TOGETHER:
  1. PARA classification (actionability)
  2. Instance tag (which specific one)

P - Projects (deadline, finite scope)
   #para/project + #project/pai
   Shortcut: #project → #para/project

A - Areas (ongoing responsibility)
   #para/area + #lifeos/health
   Shortcut: #area → #para/area

R - Resources (reference material)
   #para/resource + #topic/ai
   Shortcut: #resource → #para/resource

A - Archive (inactive/completed)
   #para/archive
   Shortcut: #archive → #para/archive
...
const CLI_HELP_ZETTELKASTEN = `
📝  ZETTELKASTEN METHOD

"Without the CONNECT layer, you have a filing cabinet.
 With it, you have a thinking partner."

THE TRANSFORMATION STACK:
  CAPTURE → STORE → CONNECT → CREATE
                       ↑
               The magic happens here

NOTE TYPES (intellectual maturity):

  #type/fleeting    - Raw capture
                      Quick thoughts, voice memos
                      Process within 24-48h or delete

  #type/literature  - Summarized
                      From external sources
                      Key insights extracted

  #type/permanent   - Your ideas
...
KEY INSIGHT:
  Fleeting notes are temporary. They either evolve into
  permanent knowledge or get deleted. This forces
  processing, not hoarding.

📖 How to Take Smart Notes - Sönke Ahrens
`;

const CLI_HELP_MAIN = `
🤖  PAI INGEST BOT - Quick Reference

CONTENT TYPES:
  📝 TEXT       Just send, auto-saves with AI tags
  🎤 VOICE      Whisper transcription (m4a, mp3, wav, ogg)
  📄 DOCUMENTS  /attach keeps filename (pdf, docx, txt, md)
  📷 PHOTOS     /describe or /mermaid (jpg, png, heic, webp)
  🔗 URLS       /fetch (raw) | /clean (strip HTML) | /article (+ wisdom)

FILING (any content):
  /file         Rename TYPE-DATE-TITLE, sync to Dropbox
  /name Title   Set custom note/archive name
  /date VALUE   Backdate document (see formats below)

DOCUMENT DATE (for historical documents):
  /date 2024-06-15         ISO format
  /date 15/06/2024         DD/MM/YYYY format
  /date yesterday          Relative (also: last month, last week)
  /date 15th June          Natural language
  [date:2024-06-15]        Metadata format (alternative)
  --date 2024-06-15        CLI flag for ingest direct/file
...
LARGE FILES (>20MB):
  ingest file recording.m4a -t meeting   # Copy to iCloud, metadata to Telegram
  ingest file doc.pdf --pipeline /archive
  Requires: Terminal → Full Disk Access (for iCloud folder)

MORE HELP:
  ingest help tags         Tag taxonomy (11 dimensions)
  ingest help para         PARA method explained
  ingest help zettelkasten Zettelkasten method explained
`;
...
function handleHelp(args: string[]) {
  ...
  switch (subcommand) {
    case "tags":
      console.log(CLI_HELP_TAGS);
      break;
    case "para":
      console.log(CLI_HELP_PARA);
      break;
    case "zettelkasten":
    case "zk":
      console.log(CLI_HELP_ZETTELKASTEN);
      break;
    case undefined:
      console.log(CLI_HELP_MAIN);
      break;
    default:
      console.log(`Unknown help topic: ${subcommand}`);
      console.log("\nAvailable topics:");
      console.log("  ingest help          Quick reference");
      console.log("  ingest help tags     Tag taxonomy");
      console.log("  ingest help para     PARA method");
      console.log("  ingest help zettelkasten  Zettelkasten method");
  }
}
...
async function handleProcess(
  ...
  for (const msg of messages) {
    ...
    if (isHelpCommand(msg)) {
      const subcommand = extractHelpSubcommand(msg);
      console.log(`[${msg.message_id}] Help request${subcommand ? `: ${subcommand}` : ""}`);
      if (!dryRun) {
        await sendHelpResponse(msg.message_id, subcommand);
    ...
    try {
      ...
      // Cleanup drop buffer file after successful save (SMS-300)
      const firstContent = contents[0];
      if (firstContent?.isDropBufferFile && firstContent?.originalFilePath) {
        await cleanupDropFolderFile(firstContent.originalFilePath);
      }
      ...
      // Send error reply to inbox (replying to failed message)
      await sendErrorResponse(msg.message_id, errorMsg);
...
async function handleDirect(
  ...
  if (args.includes("--help") || args.includes("-h")) {
    console.log(`ingest direct - Send content to Telegram inbox

USAGE:
  ingest direct <file>                     # Send file to inbox
  ingest direct --text "content"           # Send text content
  pbpaste | ingest direct                  # Send from clipboard/stdin

OPTIONS:
  --caption, -c <text>   Full caption (like Telegram: #tags @mentions /commands)
  --tags, -t <tags>      Add tags (comma-separated, shorthand for #tag)
  --pipeline <name>      Force pipeline (/archive, /wisdom, /attach, /file)
  --scope, -s <scope>    Set scope (private/work/public)
  --name, -n <name>      Override filename
  --date, -d <date>      Set document date
  --dry-run              Show what would be sent without doing it

EXAMPLES:
  ingest direct document.pdf                        # Send file
  ingest direct notes.md -t project/pai,topic/ai    # With tags
  ingest direct --text "Quick note" -t inbox        # Text with tag
  pbpaste | ingest direct -t meeting                # From clipboard
  ingest direct report.pdf --pipeline /archive      # Force pipeline
`);
    return;
  }
...
/**
 * Handle the 'file' command - copy large file to iCloud drop folder and send metadata to Telegram
 * This enables ingestion of files larger than Telegram's 20MB limit (SMS-300)
 *
 * Usage mirrors iOS shortcuts: file goes to iCloud, metadata goes to Telegram,
 * daemon correlates and processes.
 */
async function handleFile(
  ...
  // Handle --help flag
  if (args.includes("--help") || args.includes("-h")) {
    console.log(`ingest file - Send large files via iCloud drop folder

USAGE:
  ingest file <file>                       # Copy to iCloud drop + notify

OPTIONS:
  --caption, -c <text>   Full caption (like Telegram: #tags @mentions /commands)
  --tags, -t <tags>      Add tags (comma-separated)
  --pipeline <name>      Force pipeline (/archive, /wisdom, etc.)
  --scope, -s <scope>    Set scope (private/work/public)
  --name, -n <name>      Override filename
  --date, -d <date>      Set document date
...
HOW IT WORKS:
  1. File is copied to iCloud drop folder (syncs to iOS)
  2. Metadata sent to Telegram with [file:name] marker
  3. Daemon correlates file + metadata, processes normally

EXAMPLES:
  ingest file recording.m4a                    # Large audio file
  ingest file meeting.m4a -t meeting,standup   # With tags
  ingest file doc.pdf --pipeline /archive      # Force pipeline
`);
    return;
  }

  const { sendToInbox } = await import("./lib/telegram");
  const { basename, resolve } = await import("path");
  const { existsSync, mkdirSync, copyFileSync } = await import("fs");
  const { join } = await import("path");

  // Parse arguments (ADR-001 flags, same as handleDirect)
  let filePath: string | undefined;
  let caption: string | undefined;
  let tags: string | undefined;           // --tags, -t (comma-separated)
  let forcePipeline: string | undefined;  // --pipeline (e.g., /archive, /wisdom)
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--file" || arg === "-f") {
      ...
      // Positional file argument: `ingest file recording.m4a`
      filePath = arg;
    }
  }
  ...
  if (!filePath) {
    ...
    console.log("\nUsage:");
    console.log("  ingest file recording.m4a                    # Large file to iCloud drop");
    console.log("  ingest file meeting.m4a -t meeting,standup   # With tags");
    console.log("  ingest file doc.pdf --pipeline /archive      # Force pipeline");
    console.log("");
    console.log("Options:");
    console.log("  --caption, -c    Full caption (like Telegram: #tags @mentions /commands ~scope)");
    console.log("  --tags, -t       Add tags (comma-separated, shorthand for #tag in caption)");
    ...
    console.log("  1. File is copied to iCloud drop folder (syncs to iOS)");
    console.log("  2. Metadata sent to Telegram with [file:name] marker");
    console.log("  3. Daemon correlates file + metadata, processes normally");
    console.log("");
    console.log("This bypasses Telegram's 20MB limit by using iCloud as transport.");
    process.exit(1);
  }
  ...
  // Get config for drop folder path
  const config = getConfig();
  const dropFolder = config.dropFolderPath;

  // Ensure drop folder exists
  if (!existsSync(dropFolder)) {
    if (dryRun) {
      console.log(`Would create drop folder: ${dropFolder}`);
    } else {
  ...
  const destFilename = overrideName || originalFilename;
  const destPath = join(dropFolder, destFilename);

  // Build message with metadata markers (same format as iOS shortcuts)
  const messageParts: string[] = [];

  // File marker (required)
  messageParts.push(`[file:${destFilename}]`);

  // Source metadata
  messageParts.push("[source:cli]");

  // Device model
  const deviceModel = await getDeviceModel();
  messageParts.push(`[device:${deviceModel.toLowerCase()}]`);

  // Username
  const user = (process.env.USER || process.env.USERNAME || "unknown").toLowerCase();
  messageParts.push(`[user:${user}]`);

  // Pipeline command
  if (forcePipeline) {
    const pipelineCmd = forcePipeline.startsWith("/") ? forcePipeline : `/${forcePipeline}`;
    messageParts.push(pipelineCmd);
  }
  ...
  // Scope
  if (scope) {
    messageParts.push(`~${scope}`);
  }

  // Document date
  if (documentDate) {
    messageParts.push(`[date:${documentDate}]`);
  }

  // User-provided caption
  if (caption) {
    messageParts.push(caption);
  }

  const finalMessage = messageParts.join(" ");

  // Display what we're doing
  console.log(`\n📁 Large File → iCloud Drop Buffer`);
  console.log(`${"─".repeat(50)}`);
  console.log(`  Source: ${filePath}`);
  console.log(`  Drop folder: ${destPath}`);
  ...
  if (dryRun) {
    ...
    console.log(`  1. Copy file to: ${destPath}`);
    console.log(`  2. Send to Telegram: ${finalMessage}`);
    console.log(`  3. Daemon correlates and processes`);
  ...
  try {
    ...
    // Step 1: Copy file to drop folder
    console.log(`📤 Copying to iCloud drop folder...`);
    copyFileSync(filePath, destPath);
    console.log(`   ✅ Copied: ${destFilename}`);

    // Step 2: Send metadata to Telegram
    console.log(`📨 Sending metadata to Telegram...`);
    const result = await sendToInbox(finalMessage);
    console.log(`   ✅ Message ID: ${result.message_id}`);

    console.log(`\n✅ File queued for processing`);
    console.log(`   File will sync via iCloud to daemon`);
    console.log(`   Daemon correlates [file:${destFilename}] marker with drop folder`);
    console.log(`\n💡 Ensure \`ingest watch\` is running to process.`);

    if (verbose) {
      console.log(`\nDebug info:`);
      console.log(`  Drop folder: ${dropFolder}`);
      console.log(`  Drop enabled: ${config.dropFolderEnabled}`);
      console.log(`  Cleanup: ${config.dropFolderCleanup}`);
      console.log(`  Timeout: ${config.dropFolderTimeout}s`);
    }
  } catch (error) {
    console.error(`\n❌ Error: ${error instanceof Error ? error.message : error}`);
    process.exit(1);
  ...
  if (retryFailed && processNow) {
    ...
    for (const msgInfo of retryable) {
      ...
      try {
        ...
        await sendErrorResponse(msgInfo.messageId, errorMsg);
        console.log(`    ❌ Failed: ${errorMsg}`);
        failed++;
      }
    }
    ...
    try {
      ...
      await sendErrorResponse(messageId, errorMsg);
 ...
 *   ingest search --person john_smith   # Person filter
...
async function handleSearch(args: string[], verbose: boolean) {
  ...
  if (!searchText && !tag && !person) {
    ...
    console.log("  ingest search --person john_smith   # Person filter");
 ...
 *
 * Usage:
 *   ingest query "what did I discuss with John?"   # Semantic search
 ...
 *   ingest query --person john_smith               # Filter by person
 */
async function handleQuery(args: string[], verbose: boolean) {
  ...
  if (!searchText && !docType && !category && !year && !tag && !person) {
    ...
    console.log("Usage:");
    console.log('  ingest query "what did I discuss with John?"  # Semantic search');
    console.log('  ingest query "project PAI status"             # Natural language');
    console.log("  ingest query --tag project/pai                # Filter by tag");
    console.log("  ingest query --person john_smith              # Filter by person");
