...
export interface IngestConfig {
  ...
  // iCloud Drop Buffer (for files >20MB) - SMS-300
  dropFolderEnabled: boolean;      // Enable drop folder feature (default: false)
  dropFolderPath: string;          // iCloud drop folder path
  dropFolderCleanup: boolean;      // Delete files after processing
  dropFolderTimeout: number;       // Seconds to wait for file sync
  dropFolderPollInterval: number;  // Seconds between existence checks
  // Vision API (for photo processing)
  visionEnabled: boolean;          // Enable Vision API processing (default: false)
}
...
/**
 * Load environment from multiple sources (priority: env > PAI_DIR > fabric)
 */
...
export function getConfig(): IngestConfig {
  ...
  // iCloud Drop Buffer configuration (SMS-300)
  const defaultDropFolderPath = join(
    homedir(),
    "Library",
    "Mobile Documents",
    "com~apple~CloudDocs",
  ...
  const dropFolderPath = (
    process.env.DROP_FOLDER_PATH ||
    env.DROP_FOLDER_PATH ||
    defaultDropFolderPath
  ).replace(/^~/, homedir());

  // Drop folder is opt-in (default: false) - users can stick with Telegram's 20MB limit
  const dropFolderEnabled =
    (process.env.DROP_FOLDER_ENABLED || env.DROP_FOLDER_ENABLED || "false") === "true";

  const dropFolderCleanup =
    (process.env.DROP_FOLDER_CLEANUP || env.DROP_FOLDER_CLEANUP || "true") === "true";

  const dropFolderTimeout = parseInt(
    process.env.DROP_FOLDER_TIMEOUT || env.DROP_FOLDER_TIMEOUT || "3600",
    10
  );

  const dropFolderPollInterval = parseInt(
    process.env.DROP_FOLDER_POLL_INTERVAL || env.DROP_FOLDER_POLL_INTERVAL || "5",
    10
  );

  // Vision API is opt-in (default: false) - enable with VISION_ENABLED=true
  const visionEnabled =
    (process.env.VISION_ENABLED || env.VISION_ENABLED || "false") === "true";
  ...
  cachedConfig = {
    ...
    // iCloud Drop Buffer (SMS-300)
    dropFolderEnabled,
    dropFolderPath,
    dropFolderCleanup,
    dropFolderTimeout,
    dropFolderPollInterval,
    // Vision API
    visionEnabled,
  };
