...
import { generateTitleFromContent, generateTitleFromTags } from "./taxonomy";
import {
...
import { processTags, type TagProcessingOptions } from "./taxonomy-integration";
import type { SourceType } from "../../ctx/lib/inference";

const execAsync = promisify(exec);

/**
 * Map metadata source values to SourceType
 *
 * Metadata sources come from iOS Shortcuts like:
 * - [source:cli] → "cli"
 * - [source:clipboard-share] → "clipboard"
 * - [source:voice-memo] → "telegram" (voice is content type, not source)
 * - [source:photo-capture] → "telegram" (photo is content type, not source)
 *
 * Default to "telegram" for anything unrecognized since everything
 * passes through Telegram as the transport layer.
 */
function mapMetadataSourceToSourceType(metadataSource: string | undefined): SourceType {
  if (!metadataSource) return "telegram";

  const normalized = metadataSource.toLowerCase().trim();

  // Direct mappings
  if (normalized === "cli") return "cli";
  if (normalized === "clipboard" || normalized === "clipboard-share") return "clipboard";
  if (normalized === "web") return "web";

  // Everything else defaults to telegram (the transport layer)
  // Content types like "voice-memo", "photo-capture" should use contentFormat instead
  return "telegram";
}
...
// =============================================================================
// Drop Folder File Tracking (SMS-362)
// =============================================================================

/**
 * Cache of recently processed drop folder files (SMS-362)
 * Maps filename -> timestamp when it was cleaned up
 * Used to fail fast when a duplicate reference tries to wait for a file that was already processed
 */
const recentlyProcessedFiles = new Map<string, number>();

/** How long to remember processed files (in ms) - default 5 minutes */
const RECENTLY_PROCESSED_WINDOW_MS = 5 * 60 * 1000;

/**
 * Record a file as recently processed (called after cleanup)
 * @param filename - The filename (not full path) that was processed
 */
export function recordProcessedFile(filename: string): void {
  const now = Date.now();
  recentlyProcessedFiles.set(filename, now);

  // Clean up old entries
  for (const [file, timestamp] of recentlyProcessedFiles) {
    if (now - timestamp > RECENTLY_PROCESSED_WINDOW_MS) {
      recentlyProcessedFiles.delete(file);
    }
  }
}

/**
 * Check if a file was recently processed (within the tracking window)
 * @param filename - The filename to check
 * @returns True if the file was processed recently
 */
export function wasRecentlyProcessed(filename: string): boolean {
  const timestamp = recentlyProcessedFiles.get(filename);
  if (!timestamp) return false;

  return Date.now() - timestamp < RECENTLY_PROCESSED_WINDOW_MS;
}

/**
 * Resolve a file from the iCloud Drop Buffer folder (SMS-300)
 * Polls for file existence with configurable timeout for iCloud sync delay
 * @param filename - Filename or relative path from [file:] marker
 * @returns Full local path to the file, or null if not found within timeout
 */
export async function resolveDropFolderFile(filename: string): Promise<string | null> {
  const config = getConfig();
  const fullPath = join(config.dropFolderPath, filename);

  console.log(`  Resolving drop folder file: ${filename}`);
  console.log(`    Full path: ${fullPath}`);

  // Check if file exists immediately
  if (existsSync(fullPath)) {
    console.log(`    File found immediately`);
    return fullPath;
  }

  // SMS-362: Check if this file was recently processed (already cleaned up)
  // This prevents blocking for up to 1 hour when a duplicate reference comes in
  if (wasRecentlyProcessed(filename)) {
    console.warn(`    File was already processed and cleaned up (SMS-362 fix)`);
    console.warn(`    Skipping - file not found (already processed)`);
    return null;
  }

  // Poll for file with timeout (iCloud sync may take time)
  const startTime = Date.now();
  const timeoutMs = config.dropFolderTimeout * 1000;
  const pollIntervalMs = config.dropFolderPollInterval * 1000;

  console.log(`    Waiting for iCloud sync (timeout: ${config.dropFolderTimeout}s, poll: ${config.dropFolderPollInterval}s)...`);
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    if (existsSync(fullPath)) {
      ...
      console.log(`    File synced after ${elapsed}s`);
      return fullPath;
    }
  }

  console.warn(`    File not found after ${config.dropFolderTimeout}s timeout`);
  return null;
}

/**
 * Image file extensions - shared constant to avoid duplication
 * Used for content type detection and to skip pandoc text extraction for images
 */
const IMAGE_EXTENSIONS = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'heic', 'heif', 'bmp', 'tiff', 'tif', 'svg'];

/**
 * Determine content type from file extension (for drop folder files)
 * @param filename - Filename to analyze
 * @returns ContentType based on extension
 */
export function getContentTypeFromFilename(filename: string): ContentType {
  const ext = filename.toLowerCase().split('.').pop() || '';

  // Audio extensions
  if (['m4a', 'mp3', 'wav', 'ogg', 'aac', 'flac'].includes(ext)) {
    return 'audio';
  }

  // Voice memo (typically m4a but could be distinct)
  if (ext === 'voice' || ext === 'oga') {
    return 'voice';
  }
  ...
  // Document extensions (including PDF)
...
/**
 * Clean up a file from the drop folder after successful processing (SMS-300)
 * Only deletes if DROP_FOLDER_CLEANUP is enabled
 * Records the file as recently processed to prevent blocking on duplicate references (SMS-362)
 */
export async function cleanupDropFolderFile(filePath: string): Promise<boolean> {
  const config = getConfig();

  // Extract filename from full path for tracking (SMS-362)
  const filename = basename(filePath);

  if (!config.dropFolderCleanup) {
    console.log(`    Drop folder cleanup disabled, keeping: ${filePath}`);
    // Still record as processed even if not deleted, to prevent duplicate processing
    recordProcessedFile(filename);
    return false;
  }

  try {
    await unlink(filePath);
    console.log(`    Cleaned up drop folder file: ${filePath}`);
    // Record the file as recently processed (SMS-362)
    recordProcessedFile(filename);
    return true;
  } catch (err) {
    console.warn(`    Failed to cleanup drop folder file: ${err}`);
    // Still record as processed even if cleanup failed, to prevent re-processing
    recordProcessedFile(filename);
    return false;
  }
}
...
export interface ProcessedContent {
  ...
  isDropBufferFile?: boolean; // True if file came from iCloud drop folder (SMS-300)
  messageDate?: number;      // Original Telegram message timestamp (Unix)
  messageId?: number;        // Telegram message ID (for unique filenames)
...
export interface SourceMetadata {
  source?: string;          // e.g., "clipboard-share", "photo-capture", "voice-memo", "shortcut"
  device?: string;          // e.g., "iphone", "ipad", "mac"
  user?: string;            // e.g., "john", "jane"
  ...
  dropFile?: string;        // iCloud Drop Buffer file reference (SMS-300)
}
 ...
 */
/**
 * Priority level for notes (signal importance)
 */
...
export interface InlineHints {
  ...
  priority?: PriorityType;  // ~priority, ~important (high), ~low
  metadata: SourceMetadata; // [key:value] pairs for source tracking
  cleanedContent: string;   // Content with hints removed
  customName?: string;      // /name <value> - explicit note/file name
  customDate?: string;      // /date <YYYY-MM-DD> - explicit document date
}

/**
 * Pipeline types for Layer 2 routing
 */
export type PipelineType = "note" | "filing" | "attach" | "default";
...
function sanitizeArchiveFilename(text: string): string {
  return text
    .replace(/[<>:"/\\|?*'`''""]/g, "")  // Remove invalid chars including smart quotes
...
export function parseDictatedDate(text: string): string | undefined {
  ...
  // IMPORTANT: We need to distinguish between filing intent and historical references
  // - "dated 15th June" → filing intent (always extract)
  // - "this is from June 15" → filing intent (extract)
  // - "document from June 15" → filing intent (extract)
  // - "the transaction from June 15" → historical reference (DON'T extract)
  //
  // The pattern uses a lookbehind to ensure "from" is preceded by filing context words
  // or appears at the start of text (after optional non-word chars like "[")

  // Pattern: "dated 15th June 2024" - always filing intent
  const datedPattern = /dated?\s+(?:the\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|june?|july?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s*(\d{4})?/i;
  const datedMonthFirstPattern = /dated?\s+(?:the\s+)?(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|june?|july?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(\d{1,2})(?:st|nd|rd|th)?\s*,?\s*(\d{4})?/i;

  // Pattern: "from" with filing context - only extract if preceded by filing words or at start
  // Filing context words: "this", "document", "receipt", "invoice", "contract", "file", "letter", "bill"
  const fromWithContextPattern = /(?:^|(?:this\s+(?:is\s+)?|document\s+|receipt\s+|invoice\s+|contract\s+|file\s+|letter\s+|bill\s+))from\s+(?:the\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|june?|july?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s*(\d{4})?/i;
  const fromMonthFirstWithContextPattern = /(?:^|(?:this\s+(?:is\s+)?|document\s+|receipt\s+|invoice\s+|contract\s+|file\s+|letter\s+|bill\s+))from\s+(?:the\s+)?(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|june?|july?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(\d{1,2})(?:st|nd|rd|th)?\s*,?\s*(\d{4})?/i;
  ...
  // Try "dated" patterns first (explicit filing intent, always match)
  let match = datedPattern.exec(lowerText);
  if (match) {
  ...
  match = datedMonthFirstPattern.exec(lowerText);
  if (match) {
  ...
  // Try "from" patterns with filing context (only match with appropriate context)
  match = fromWithContextPattern.exec(lowerText);
  if (match) {
    const day = parseInt(match[1], 10);
    const monthStr = match[2].toLowerCase();
    const month = monthNames[monthStr.slice(0, 3)] ?? monthNames[monthStr];
  ...
  match = fromMonthFirstWithContextPattern.exec(lowerText);
  if (match) {
    const monthStr = match[1].toLowerCase();
    const day = parseInt(match[2], 10);
    const month = monthNames[monthStr.slice(0, 3)] ?? monthNames[monthStr];
  ...
  // Pattern: "date 2024-06-15" or "dated 2024-06-15" (explicit intent only)
  const isoPattern = /dated?\s+(\d{4})-(\d{2})-(\d{2})/i;
  match = isoPattern.exec(lowerText);
  if (match) {
    return `${match[1]}-${match[2]}-${match[3]}`;
  }

  // Pattern: "date 15/06/2024" or "dated 15/6/24" (explicit intent only)
  const slashPattern = /dated?\s+(\d{1,2})\/(\d{1,2})\/(\d{2,4})/i;
  ...
  // Pattern: "last month" / "last month's" (relative date with implicit filing context)
  // These are kept without "from" requirement as they're unambiguous filing intent
  if (/last\s+month/i.test(lowerText)) {
    const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    return formatLocalDate(lastMonth);
  }

  // Pattern: "last week"
  if (/last\s+week/i.test(lowerText)) {
    const lastWeek = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    return formatLocalDate(lastWeek);
  }

  // Pattern: "yesterday"
  if (/yesterday/i.test(lowerText)) {
...
export function determinePipeline(commands: string[]): PipelineType {
  if (commands.includes("file")) return "filing";
  if (commands.includes("archive")) {
    console.warn("⚠️  DEPRECATED: /archive is deprecated, use /file instead");
    return "filing";  // /archive redirects to filing (backward compatibility)
  }
  if (commands.includes("receipt")) return "filing";  // Legacy: /receipt → /filing
 ...
 * Maps natural language phrases to filing pipeline
...
export function detectDictatedPipelineIntent(caption: string | undefined): { pipeline: PipelineType; metadata?: { type?: string; category?: string } } | null {
  ...
  for (const pattern of receiptPatterns) {
    if (pattern.test(lowerCaption)) {
      console.log(`  Detected dictated pipeline intent: "filing" (receipt/invoice) from caption`);

      // Detect document type for filing pipeline
      ...
      return { pipeline: "filing", metadata: { type, category } };
    }
  }

  // Filing patterns with optional category detection (supports both "file this" and "archive this" for backward compat)
  ...
  for (const pattern of filingPatterns) {
    if (pattern.test(lowerCaption)) {
      console.log(`  Detected dictated pipeline intent: "filing" from caption`);
...
Guidelines:
- "filing" pipeline: contracts, important documents, legal papers, receipts, invoices
- "note" pipeline: ideas, meeting notes, general text, articles
...
Return ONLY valid JSON, no markdown code blocks.`;
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-5-mini",  // Fast, cost-efficient model for intent extraction
        messages: [
          { role: "system", content: "You extract document processing intent from natural language. Return only valid JSON." },
          { role: "user", content: prompt },
        ],
        max_completion_tokens: 500,  // Allow room for JSON response
      }),
    });
    ...
    // Validate pipeline
    const validPipelines = ["note", "filing"];
 ...
 *
 * Example message:
 *   "#project/pai @john_smith /meeting-notes
...
export function mergeHints(base: InlineHints, additional: InlineHints): InlineHints {
  return {
    ...
    priority: additional.priority || base.priority,  // Additional takes precedence
    metadata: { ...base.metadata, ...additional.metadata },
    cleanedContent: additional.cleanedContent || base.cleanedContent,
    customName: additional.customName || base.customName,  // Additional takes precedence
  };
}
...
function extractHints(text: string, spokenMode: boolean): InlineHints {
  ...
  // Date extraction is ONLY from explicit /date command, never from transcript content
  // Supports NLP: /date yesterday, /date last month, /date 15th June
  // Supports formats: /date 2024-06-15, /date 15/06/2024
  // See SMS-361 for rationale (transcript content should not trigger date extraction)
  ...
  while ((match = metadataPattern.exec(workingText)) !== null) {
    ...
    switch (key) {
      ...
      case "date":
        ...
        break;
      case "file":
        // iCloud Drop Buffer file reference (SMS-300)
        // Supports: [file:meeting.m4a] or [file:recordings/standup.m4a]
        metadata.dropFile = value;
        console.log(`  Drop folder file: ${value}`);
        break;
    }
  }

  // Extract hashtags: #tag or #project/name (supports / and . in tags)
  // Must be at word boundary or start of line
  // Examples: #human3.0, #project/pai, #v2.1-beta
  const tagPattern = /(?:^|\s)#([a-zA-Z][a-zA-Z0-9_./-]*)/gm;
  while ((match = tagPattern.exec(workingText)) !== null) {
    tags.push(match[1]);
  }

  // Map shorthand tags to canonical forms
  // #prio → priority/high, #p1 → priority/high, #p2 → priority/low
  ...
  for (let i = 0; i < tags.length; i++) {
    const mapped = tagMappings[tags[i].toLowerCase()];
    if (mapped) {
      console.log(`  Mapped tag #${tags[i]} → ${mapped}`);
      tags[i] = mapped;
    }
  }

  // Extract mentions: @name, @firstname_lastname, @first-last, or @First Last
  // Must be at word boundary or start of line
  // Supports: @john_smith, @mike-wright, @John, @John Smith (normalized to lowercase with underscores)
  const mentionPattern = /(?:^|\s)@([a-zA-Z][a-zA-Z0-9_-]*(?:[\s-][a-zA-Z][a-zA-Z0-9_-]*)?)/gm;
  while ((match = mentionPattern.exec(workingText)) !== null) {
    // Normalize: "John Smith" → "john_smith", "mike-wright" → "mike_wright", "John" → "john"
    const normalized = match[1].replace(/[\s-]+/g, "_").toLowerCase();
    people.push(normalized);
  }

  // Extract commands: /command ONLY at start of line, after whitespace, or after ]
  // NOT in URLs (http://..., https://...)
  // Supports: "[metadata]/mermaid" (no space after ]) from shortcuts
  ...
  // Extract priority: ~priority, ~important (high), ~low
  let priority: PriorityType | undefined;
  const priorityPattern = /(?:^|\s)~(priority|important|high|low)\b/gi;
  while ((match = priorityPattern.exec(workingText)) !== null) {
    const hint = match[1].toLowerCase();
    if (!priority) {
      // Map hints to priority values
      if (hint === "priority" || hint === "important" || hint === "high") {
        priority = "high";
      } else if (hint === "low") {
        priority = "low";
      }
      console.log(`  Extracted priority hint: "${priority}"`);
    }
  }

  // Natural language priority detection:
  // "this is important" / "high priority" / "top priority" → high
  // "low priority" / "not urgent" → low
  if (!priority) {
    if (/\b(this\s+is\s+important|high\s+priority|top\s+priority|really\s+important|super\s+important)\b/i.test(workingText)) {
      priority = "high";
      console.log(`  Detected dictated priority intent: "high" from natural language`);
    } else if (/\b(low\s+priority|not\s+urgent|not\s+important)\b/i.test(workingText)) {
      priority = "low";
      console.log(`  Detected dictated priority intent: "low" from natural language`);
    }
  }

  // Extract /name command value: /name <value until next command, tag, or newline>
  // Example: "/name My Custom Title" → customName = "My Custom Title"
  // Supports: "[metadata]/name Title" (no space after ]) from shortcuts
  ...
  // Extract /date command value: /date <value until next command, tag, or newline>
  // Example: "/date 2024-06-15" → customDate = "2024-06-15"
  // Supports NLP: "/date yesterday", "/date last month", "/date 15th June"
  // Supports formats: "/date 15/06/2024", "/date 2024-06-15"
  let customDate: string | undefined;
  const dateCommandPattern = /(?:^|\s|\])\/date\s+([^#@\/\[\]\n]+)/i;
  const dateMatch = workingText.match(dateCommandPattern);
  if (dateMatch) {
    const dateValue = dateMatch[1].trim();
    if (dateValue) {
      // Parse the date value (supports NLP and traditional formats)
      const parsedDate = parseDictatedDate(dateValue);
      if (parsedDate) {
        customDate = parsedDate;
        metadata.date = parsedDate;
        console.log(`  Custom date: ${customDate}`);
      } else {
        console.log(`  Warning: Could not parse date value: ${dateValue}`);
      }
    }
  }
  ...
  let cleaned = workingText
    ...
    .replace(/(?:^|\s)@[a-zA-Z][a-zA-Z0-9_-]*(?:[\s-][a-zA-Z][a-zA-Z0-9_-]*)?,?/gm, " ")  // @First Last, @first-last, or @name
    .replace(/(?:^|\s|\])\/name\s+[^#@\/\[\]\n]+/gi, " ")  // Remove /name <value> (supports ]/name from shortcuts)
    .replace(/(?:^|\s|\])\/date\s+[^#@\/\[\]\n]+/gi, " ")  // Remove /date <value> (supports ]/date from shortcuts)
    .replace(/(?:^|\s)\/[a-zA-Z][a-zA-Z0-9_-]*(?=\s|$)/gm, " ")  // Remove other /commands
    .replace(/(?:^|\s)~(private|work)\b/gi, " ")  // Remove ~scope hints
    .replace(/(?:^|\s)~(priority|important|high|low)\b/gi, " ")  // Remove ~priority hints
...
export async function processMessage(
  ...
  switch (contentType) {
    ...
    case "audio":
      ...
      if (audioResult.spokenHints) {
        ...
        // Merge priority from spoken hints (e.g., "this is important" in transcript)
        if (merged.priority && !hints.priority) {
          hints.priority = merged.priority;
        }
    ...
    case "document":
      // Check pipeline: /file for rename+Dropbox, /attach (default) for original filename
      const docPipeline = determinePipeline(validCommands);
      const isExplicitFiling = docPipeline === "filing";
      const keepOriginalFile = isExplicitFiling || docPipeline === "attach" || docPipeline === "default";
      ...
      if (isPdf && !hasOcrCommand) {
        ...
        // Check if filename indicates filing (e.g., "RECEIPT - 20211108 - ...")
        if (shouldPreserveArchiveName(docFilename)) {
          console.log(`  PDF filing mode - pre-named filing file`);
        } else {
        ...
        rawContent = hints.cleanedContent || `PDF document: ${docFilename}`;
        ...
        // Default to "attach" is applied later if no filing intent detected
      } else {
        // Non-PDF or PDF with /ocr: run full extraction
        if (isPdf && hasOcrCommand) {
          console.log(`  PDF OCR mode - extracting text with marker`);
        }
        const docResult = await processDocument(message, config.tempDir, keepOriginalFile, hints);
    ...
    case "photo":
      // Check if filing pipeline to preserve original file for Dropbox sync
      ...
      const keepPhotoForFiling = photoPipeline === "filing";
      const photoResult = await processPhoto(message, config.tempDir, hints, keepPhotoForFiling);
      rawContent = photoResult.content;
      suggestedTitle = photoResult.title;
      // Store original path for Dropbox sync (will be used later if filing pipeline)
      if (photoResult.filePath && keepPhotoForFiling) {
    ...
    case "text":
    default:
      // Check for iCloud Drop Buffer file reference (SMS-300)
      // Only resolve file if DROP_FOLDER_ENABLED=true, otherwise just extract metadata
      if (hints.metadata.dropFile && config.dropFolderEnabled) {
        console.log(`  iCloud Drop Buffer detected: ${hints.metadata.dropFile}`);
        const dropFilePath = await resolveDropFolderFile(hints.metadata.dropFile);

        if (!dropFilePath) {
          // File not found within timeout - fail gracefully
          return {
            success: false,
            error: `Drop folder file not found: ${hints.metadata.dropFile}`,
          };
        }

        // Store the resolved path for later processing (attachments, cleanup)
        (message as any)._originalFilePath = dropFilePath;
        (message as any)._dropBufferFile = true;

        // Determine content type from file extension
        const dropContentType = getContentTypeFromFilename(hints.metadata.dropFile);
        console.log(`    File type: ${dropContentType}`);

        // Route to appropriate processor based on file type
        switch (dropContentType) {
          case "audio":
          case "voice":
            // Create a synthetic message with audio properties for processAudio
            const audioMessage: TelegramMessage = {
              ...message,
              audio: {
                file_id: "", // Not used - we have local path
                duration: 0,
                file_name: basename(dropFilePath),
              },
            };
            // Process audio from local file path
            const dropAudioResult = await processAudioFromPath(
              dropFilePath,
              hints.cleanedContent || "",
              config.tempDir
            );
            rawContent = dropAudioResult.content;
            suggestedTitle = dropAudioResult.title;
            // Merge spoken hints from transcript
            if (dropAudioResult.spokenHints) {
              const matchResult = matchTranscribedHints(
                dropAudioResult.spokenHints.tags,
                dropAudioResult.spokenHints.people,
                config.vaultPath
              );
              dropAudioResult.spokenHints.tags = matchResult.tags;
              dropAudioResult.spokenHints.people = matchResult.people;
              if (matchResult.needsReview) {
                dropAudioResult.spokenHints.tags.push("pending-review");
              }
              const merged = mergeHints(hints, dropAudioResult.spokenHints);
              hints.tags = merged.tags;
              hints.people = merged.people;
              hints.metadata = merged.metadata;
            }
          ...
          case "document":
            // For documents from drop folder, just create a note with reference
            // (the attach pipeline will copy the file to vault/attachments/)
            // Use /ocr command to extract text from PDFs if needed
            if (validCommands.includes("ocr")) {
              // User explicitly requested OCR - process the document
              const dropDocResult = await processDocumentFromPath(
                dropFilePath,
                hints.cleanedContent || "",
                config.tempDir
              );
              rawContent = dropDocResult.content;
              suggestedTitle = dropDocResult.title;
            } else {
              // Default: just attach without processing (consistent with Telegram PDF behavior)
              rawContent = hints.cleanedContent || `Document: ${hints.metadata.dropFile}`;
              suggestedTitle = basename(hints.metadata.dropFile).replace(/\.[^.]+$/, "");
            }
            break;

          case "photo":
            // Process photo from drop folder with Vision API (SMS-363)
            const dropPhotoResult = await processPhotoFromPath(
              dropFilePath,
              hints.cleanedContent || "",
              validCommands
            );
            rawContent = dropPhotoResult.content;
            suggestedTitle = dropPhotoResult.title;
            break;

          default:
            rawContent = hints.cleanedContent || `File: ${hints.metadata.dropFile}`;
            suggestedTitle = basename(hints.metadata.dropFile).replace(/\.[^.]+$/, "");
        }
      } else {
        // Standard text message (no drop folder file)
  ...
  // Override title with /name command if provided
  if (hints.customName) {
    suggestedTitle = hints.customName;
  }
  ...
  if (!skipAI && config.openaiApiKey && rawContent && rawContent.length > 50) {
    try {
      // Skip slow vault scanning - taxonomy provides enough context
      // loadVaultTags scans all ~2000 files which can take 30+ seconds
      const taxonomy = loadTaxonomy();
      const existingTags = taxonomy ? taxonomy.allTagNames : [];
      ...
      // Filter out project/* and area/* tags from AI - these are PARA organizational dimensions
      // that require explicit user intent (via #project/name). AI should not assign notes to
      // projects/areas based on content similarity. Fixes over-tagging issue where AI matched
      // "project/ai-tailgating" for any AI-related content.
      const paraTagsBefore = aiTags.filter(tag =>
        tag.startsWith("project/") || tag.startsWith("area/")
      );
      if (paraTagsBefore.length > 0) {
        aiTags = aiTags.filter(tag =>
          !tag.startsWith("project/") && !tag.startsWith("area/")
        );
        console.log(`    Filtered ${paraTagsBefore.length} project/area tag(s) from AI (PARA requires explicit intent)`);
      }
    } catch (err) {
  ...
  // Step 2b: Process tags through taxonomy (validation + migration + precedence + inference)
  // Uses pluggable taxonomy system for proper tag handling
  // Now includes inference from content and source (REQ-INFER-001 through REQ-INFER-009)
  const verboseTags = process.env.VERBOSE_TAGS === "1" || validCommands.includes("verbose");
  const tagResult = await processTags(
    {
      explicitTags: hints.tags,
      personTags: hints.people,
      aiTags,
      profileTags: baseTags,
    },
    {
      verbose: verboseTags,
      autoMigrate: true,
      addDefaults: true,
      content: rawContent,
      // WHERE: source from metadata [source:cli] or default to telegram
      // Map shortcut sources (cli, clipboard-share, voice-memo) to SourceType
      source: mapMetadataSourceToSourceType(hints.metadata.source),
      contentType: contentType as "voice" | "audio" | "photo" | "document" | "text" | "url" | "screenshot", // WHAT: voice, audio, photo, document, etc.
      caption: messageText,
    }
  );
  const tags = tagResult.tags;

  // Log warnings (always, not just verbose)
  for (const warning of tagResult.warnings) {
  ...
  // Skip AI for text/url content - those should stay "default" (zettelkasten workflow)
  // AI routing is only useful for documents/photos where filing vs attach matters
  const shouldRunAIIntent = pipeline === "default" &&
    config.openaiApiKey &&
    contentType !== "text" &&
    contentType !== "url";

  if (shouldRunAIIntent) {
    ...
    if (intentResult && intentResult.confidence >= INTENT_CONFIDENCE_THRESHOLD) {
      // For documents: only apply AI intent if it's "filing" (documents default to "attach")
      // For photos: apply any AI intent
      const isDocument = contentType === "document";
      const shouldApplyIntent = !isDocument || intentResult.pipeline === "filing";
      ...
      if (shouldApplyIntent) {
        ...
        console.log(`  AI intent: ${intentResult.pipeline} ignored for document (will use attach default)`);
      }
  ...
  // Step 2b.2: Check if document filename has filing naming pattern
  // Files named like "RECEIPT - 20211108 - Description - CATEGORY.pdf" should use filing pipeline
  // Also handle drop buffer documents (which come in as text messages with [file:] markers)
  const isDropBufferDocument = (message as any)._dropBufferFile === true &&
    hints.metadata.dropFile &&
    getContentTypeFromFilename(hints.metadata.dropFile) === "document";

  if (pipeline === "default" && (contentType === "document" || isDropBufferDocument)) {
    const docFilename = message.document?.file_name || hints.metadata.dropFile || "";
    if (shouldPreserveArchiveName(docFilename)) {
      pipeline = "filing";
      console.log(`  Pre-named filing file detected: ${docFilename}`);
    } else {
      // Default to "attach" for documents if no filing intent detected
      // This runs AFTER dictated intent detection so captions like "file this" work
      pipeline = "attach";
    }
  }
  ...
  const isDropBufferFile: boolean = (message as any)._dropBufferFile === true;
  const sourceFile = message.document?.file_name || message.audio?.file_name ||
    (hints.metadata.dropFile ? basename(hints.metadata.dropFile) : undefined);

  // Note: caption-as-filename feature removed - use /rename command for custom filenames

  if (pipeline === "filing") {
    // Check if source file already has proper filing naming
    ...
    if (shouldPreserveArchiveName(sourceFilename)) {
      ...
      // Use /name command value if provided, otherwise use suggestedTitle
      const filingDescription = hints.customName || suggestedTitle;
      ...
      console.log(`  Generated filing name: ${archiveName}${dateSource !== "today" ? ` (date from ${dateSource})` : ""}`);
    }

    // Add filing-specific tags
    tags.push("filing");
  }
  ...
  // Determine scope:
  // 1. Check if scope tag already in processed tags (from #scope/private explicit tag)
  // 2. Explicit hint from user (~private, ~work)
  // 3. Auto-detect from pipeline (filing → private)
  // 4. Default to work for all other pipelines
  //
  // Security model: No tag = private (excluded from context)
  // Must explicitly have scope/work to be included in default queries
  const existingScopeTag = tags.find(t => t.startsWith("scope/"));
  const existingScope = existingScopeTag?.replace("scope/", "") as ScopeType | undefined;

  let scope: ScopeType = existingScope || hints.scope || "work";  // Check tags first, then hints
  const isFilingPipeline = pipeline === "filing";

  if (isFilingPipeline && !existingScope && !hints.scope) {
    scope = "private";
    console.log(`  Auto-set scope to "private" for ${pipeline} pipeline`);
  } else if (!existingScope && !hints.scope) {
    console.log(`  Default scope: "work"`);
  }

  // Only add scope tag if not already present
  if (!existingScopeTag) {
    tags.push(`scope/${scope}`);
  }

  // Determine priority:
  // 1. Check if priority tag already in processed tags (from #priority/high explicit tag)
  // 2. Explicit hint from user (~priority, ~important, ~low)
  // 3. No default - priority is optional
  const existingPriorityTag = tags.find(t => t.startsWith("priority/"));
  const existingPriority = existingPriorityTag?.replace("priority/", "") as PriorityType | undefined;

  // Only add priority tag if hint provided and not already present
  if (hints.priority && !existingPriorityTag) {
    tags.push(`priority/${hints.priority}`);
    console.log(`  Added priority tag: "priority/${hints.priority}"`);
  }
  ...
  // - filing pipeline (straight-through filing, no processing)
  ...
  const skipAutoPatterns = (isFilingPipeline || contentType === "document") && !hasExplicitPatternRequest;
  ...
  results.push({
    ...
    ...(isDropBufferFile && { isDropBufferFile }),
...
async function processAudio(
  ...
  try {
    ...
    // Paths from environment or .env: WHISPER_BIN, WHISPER_MODEL_DIR
    const whisperBin = process.env.WHISPER_BIN || "whisper-cpp";
    const modelDir = process.env.WHISPER_MODEL_DIR || join(homedir(), ".whisper/models");
...
async function processDocument(
  message: TelegramMessage,
  tempDir: string,
  keepOriginal: boolean = false,  // For filing pipeline: keep file for Dropbox sync
  hints?: InlineHints  // For Vision API support with image documents
  ...
  // Fallback filename from drop buffer metadata (iOS shortcuts)
  const dropFilename = hints?.metadata?.dropFile ? basename(hints.metadata.dropFile) : undefined;
  ...
  try {
    ...
    if (ext === "pdf") {
      ...
      // Image file sent as document - check for Vision API commands
      console.log(`    Image file detected (${ext})`);

      const commands = hints?.commands || [];
      const hasExplicitVisionCommand = commands.includes("describe") || commands.includes("mermaid");
      const imgConfig = getConfig();
      const shouldUseVision = imgConfig.openaiApiKey && (hasExplicitVisionCommand || imgConfig.visionEnabled);

      if (shouldUseVision) {
        console.log("    Vision API: Processing image document...");
        ...
        } else if (hints?.cleanedContent) {
          // Use cleaned caption as the prompt (requires visionEnabled)
          prompt = hints.cleanedContent;
          console.log(`    Custom prompt: ${prompt}`);
        } else {
          // Default: describe the image (requires visionEnabled)
          prompt = "Describe this image briefly. If it contains text, extract it. If it's a diagram, describe its structure.";
        }

        let visionResult = await processPhotoWithVision(filePath, prompt, imgConfig.openaiApiKey);

        // Post-process mermaid output
        ...
        // No Vision - just create placeholder
        console.log("    Skipping text extraction (no Vision API or commands)");
    ...
    // Generate title using progressive approach:
    // 1. Pattern extraction (free, deterministic) - best for TLDR, newsletters
    // 2. AI title generation (if API key set) - for complex content
    // 3. Fallback to filename or default
    const config = getConfig();
    let title: string;
    const isImageDocument = IMAGE_EXTENSIONS.includes(ext);

    if (isImageDocument) {
      // For images, use original filename (AI can't generate meaningful title from "📷 Image: X.heic")
      title = doc.file_name?.replace(/\.[^.]+$/, "") || filename.replace(/\.[^.]+$/, "") || "Image";
    } else {
      // Progressive title extraction - escalating tiers:
      // 1. Pattern extraction (free, fast, works for TLDR/newsletters)
      // 2. Tag-based title (from hints.tags + hints.people)
      // 3. AI generation (OpenAI - costs tokens)
      // 4. Filename fallbacks

      const taxonomy = loadTaxonomy();
      const titleResult = generateTitleFromContent(content, taxonomy);

      if (titleResult.method === "pattern") {
        // Tier 1: Pattern matched
        title = titleResult.title;
        console.log(`    Pattern-extracted title: "${title}" (${titleResult.pattern})`);
      } else {
        // Tier 2: Try tag-based title from hints
        const allTags = [
          ...(hints?.tags || []),
          ...(hints?.people || []).map(p => `person/${p}`),
        ];
        const tagBasedTitle = generateTitleFromTags(allTags);

        if (tagBasedTitle) {
          title = tagBasedTitle;
          console.log(`    Tag-inferred title: "${title}"`);
        } else if (config.openaiApiKey && content.length > 50) {
          // Tier 3: Fall back to AI title generation
          try {
            title = await generateAITitle(content, config.openaiApiKey, "document");
          } catch {
            title = titleResult.title || doc.file_name?.replace(/\.[^.]+$/, "") || dropFilename?.replace(/\.[^.]+$/, "") || "Document";
          }
        } else {
          // Tier 4: Filename fallbacks
          title = titleResult.title || doc.file_name?.replace(/\.[^.]+$/, "") || dropFilename?.replace(/\.[^.]+$/, "") || "Document";
        }
      }
    }
    ...
    try {
      ...
      return {
        ...
        title: doc.file_name?.replace(/\.[^.]+$/, "") || dropFilename?.replace(/\.[^.]+$/, "") || "Document",
        ...(keepOriginal && { originalPath: filePath }),
      };
    } catch {
    ...
    // Only delete original if not keeping for filing sync
    if (!keepOriginal) {
      await unlink(filePath).catch(() => {});
    }
  }
...
/**
 * Process audio from a local file path (for iCloud Drop Buffer - SMS-300)
 * Same as processAudio but works with local file instead of Telegram download
 */
async function processAudioFromPath(
  filePath: string,
  captionHint: string,
  tempDir: string
): Promise<{ content: string; title: string; spokenHints?: InlineHints }> {
  console.log(`  Processing audio from drop folder: ${filePath}`);

  const filename = basename(filePath);
  const ext = filename.split(".").pop()?.toLowerCase() || "m4a";
  const wavPath = join(tempDir, `audio_drop_${Date.now()}.wav`);
  ...
  try {
    ...
    if (config.openaiApiKey && content.length > 20) {
      try {
        ...
        title = filename.replace(/\.[^.]+$/, "") || "Voice Note";
      }
    } else {
      title = filename.replace(/\.[^.]+$/, "") || "Voice Note";
    }
    ...
    // Cleanup temp WAV file (NOT the original drop folder file - that's handled separately)
    await unlink(wavPath).catch(() => {});
  }
}

/**
 * Process document from a local file path (for iCloud Drop Buffer - SMS-300)
 * Same as processDocument but works with local file instead of Telegram download
 */
async function processDocumentFromPath(
  filePath: string,
  captionHint: string,
  tempDir: string
): Promise<{ content: string; title: string }> {
  console.log(`  Processing document from drop folder: ${filePath}`);
  ...
  try {
    ...
    // Route based on file type (same logic as processDocument)
    if (ext === "pdf") {
      // PDF: Use marker_single
      const outputDir = filePath.replace(/\.pdf$/i, "_marker_output");
      ...
      // Image file - skip text extraction (pandoc can't process images)
      console.log(`    Image file detected (${ext}), skipping text extraction`);
      content = `📷 Image: ${filename}`;
    } else {
      // Unknown format: Try pandoc plain text extraction
      const { stdout } = await execAsync(`pandoc -t plain "${filePath}"`);
    ...
    if (config.openaiApiKey && content.length > 50) {
      try {
        title = await generateAITitle(content, config.openaiApiKey, "document");
      } catch {
        title = filename.replace(/\.[^.]+$/, "") || generateTitleFromText(content);
      }
    } else {
      title = filename.replace(/\.[^.]+$/, "") || generateTitleFromText(content);
    }
...
/**
 * Process photo from a local file path (for iCloud Drop Buffer - SMS-300, SMS-363)
 * Same as processPhoto but works with local file instead of Telegram download
 */
async function processPhotoFromPath(
  filePath: string,
  captionHint: string,
  commands: string[]
): Promise<{ content: string; title: string }> {
  console.log(`  Processing photo from drop folder: ${filePath}`);

  const config = getConfig();
  const filename = basename(filePath);

  // /ocr - Tesseract OCR only
  if (commands.includes("ocr") || (!captionHint && !config.openaiApiKey)) {
    const ocrResult = await processPhotoWithOCR(filePath, captionHint);
    return ocrResult;
  }

  // Vision AI processing
  // - Explicit commands (/describe, /mermaid) always use Vision
  // - Default prompts (caption or auto-describe) require VISION_ENABLED=true
  const hasExplicitVisionCommand = commands.includes("describe") || commands.includes("mermaid");
  const shouldUseVision = config.openaiApiKey && (hasExplicitVisionCommand || config.visionEnabled);
  ...
  if (shouldUseVision) {
    ...
    if (commands.includes("describe")) {
      ...
      prompt = "Extract the diagram, flowchart, or structure from this image and convert it to Mermaid diagram syntax. IMPORTANT: Escape special characters in node labels - use quotes around text containing parentheses, brackets, or special chars (e.g., use A[\"Maximo (EAM)\"] not A[Maximo (EAM)]). Return only the Mermaid code block.";
    } else if (captionHint && captionHint.trim()) {
      // Use caption as the prompt (requires visionEnabled)
      prompt = captionHint;
      console.log(`    Custom prompt: ${prompt}`);
    } else {
      // Default: describe the image (requires visionEnabled)
    ...
    // Save image to vault and embed
    const imagePath = await saveImageToVault(filePath, config.vaultPath, Date.now());

    // Format content - clean output matching file shortcut format
    const content = `📎 **Attachment:** [[${imagePath}]]\n\n${visionResult}`;

    // Generate title
    let title: string;
    try {
      title = await generateAITitle(visionResult, config.openaiApiKey, "photo");
    } catch {
      title = filename.replace(/\.[^.]+$/, "") || "Image Analysis";
    }

    return { content, title };
  }

  // Fallback: just save image reference
  console.log("    No Vision API key, saving image reference only");
  const imagePath = await saveImageToVault(filePath, config.vaultPath, Date.now());
  const imageBasename = basename(imagePath);
  return {
    content: `📎 **Attachment:** [[${imagePath}]]\n\n📷 Image: ${imageBasename}\n\n${captionHint || "Photo from drop folder"}`,
    title: filename.replace(/\.[^.]+$/, "") || "Image",
  };
}
...
async function processPhoto(
  ...
  // Track if we should return the file path for filing sync
  ...
  try {
    ...
    if (shouldUseVision) {
      ...
      if (commands.includes("describe")) {
        ...
        // Use cleaned caption (without commands/tags) as the prompt (requires visionEnabled)
        prompt = hints.cleanedContent;
        console.log(`    Custom prompt: ${prompt}`);
      } else if (caption) {
        // Caption as prompt (requires visionEnabled)
        prompt = caption;
        console.log(`    Custom prompt: ${prompt}`);
      } else {
        // Default: describe the image (requires visionEnabled)
    ...
    // Only delete file if not keeping for filing
    if (!returnFilePath) {
      await unlink(filePath).catch(() => {});
    }
  }
...
/**
 * Convert HEIC/HEIF to JPEG for Vision API compatibility
 * Uses macOS sips command (native, no dependencies)
 * @returns Path to converted JPEG (temp file) or original path if no conversion needed
 */
async function convertHeicToJpeg(filePath: string): Promise<{ convertedPath: string; needsCleanup: boolean }> {
  const ext = filePath.split(".").pop()?.toLowerCase();

  if (ext !== "heic" && ext !== "heif") {
    return { convertedPath: filePath, needsCleanup: false };
  }

  console.log("    Converting HEIC to JPEG...");

  // Create temp file path
  const tempPath = `/tmp/pai-vision-${Date.now()}.jpg`;

  try {
    // Use macOS sips to convert (built-in, no dependencies)
    await execAsync(`sips -s format jpeg "${filePath}" --out "${tempPath}"`, { timeout: 30000 });
    console.log("    HEIC converted successfully");
    return { convertedPath: tempPath, needsCleanup: true };
  } catch (error) {
...
async function processPhotoWithVision(
  ...
  // Convert HEIC/HEIF to JPEG if needed (Vision API doesn't support HEIC)
  const { convertedPath, needsCleanup } = await convertHeicToJpeg(filePath);

  try {
    // Read image and convert to base64
    const imageData = await Bun.file(convertedPath).arrayBuffer();
    const base64Image = Buffer.from(imageData).toString("base64");

    // Detect mime type from extension
    const ext = convertedPath.split(".").pop()?.toLowerCase();
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-5.2",  // Vision API for image analysis
        messages: [
          {
            role: "user",
            content: [
    ...
    // Cleanup temp file if HEIC was converted
    if (needsCleanup) {
      try {
 ...
 * Uses taxonomy-based pattern matching before falling back to first-line extraction
 * @deprecated Use generateTitleFromContent() from taxonomy.ts instead
 */
function generateTitleFromText(text: string): string {
  const taxonomy = loadTaxonomy();
  const result = generateTitleFromContent(text, taxonomy);
  return result.title;
}
...
async function generateAITitle(
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "gpt-5-mini",  // Title generation - simpler task, cost-efficient model
      messages: [
        { role: "system", content: system },
        { role: "user", content: sample },
      ],
      max_completion_tokens: 100,  // Allow room for title response
    }),
  });
  ...
  // Clean up: remove quotes (straight and smart) and filesystem-unsafe characters, limit length
  return title
    .replace(/^["'""''`]|["'""''`]$/g, "")           // Remove leading/trailing quotes
    .replace(/[<>:"/\\|?*!@#$%^&()[\]{}'`''""]/g, "") // Remove unsafe chars including smart quotes
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 60);
}
...
Return JSON only: {"tags": ["tag1", "tag2"], "references": ["Concept A", "Related Idea"]}`;
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "gpt-5-mini",  // Tag generation - mini is fast and good enough
      messages: [
...
/**
 * Clean markdown artifacts from content (Pandoc attributes, RTF escapes, etc.)
 * Handles:
 * - {.underline}, {.class}, {style="..."} attributes from Pandoc/RTF conversion
 * - Double brackets [[text]] -> [text]
 * - Extra brackets around markdown links: [[Title](url)] -> [Title](url)
 * - TLDR tracking URLs: extract actual destination
 * - Newlines in reading time: (4 minute\nread) -> (4 minute read)
 * - Escaped newlines \\ and \n
 * - Empty brackets []
 * - Standalone brackets not part of links
 */
export function cleanMarkdownArtifacts(content: string): string {
  let cleaned = content
    // Remove Pandoc/RTF class attributes like {.underline}, {.class-name}
    .replace(/\{\.[\w-]+\}/g, '')
    // Remove style attributes like {style="..."}
    .replace(/\{[^}]*style="[^"]*"[^}]*\}/g, '')
    // Remove any remaining {...} attributes
    .replace(/\{[^}]*\}/g, '')
    // Fix extra brackets around markdown links: **[[Title](url)]** -> **[Title](url)**
    // Pattern: [[ followed by link text, ](url), then extra ]
    .replace(/\[\[([^\]]+)\]\(([^)]+)\)\]/g, '[$1]($2)')
    // Fix double brackets [[text]] -> [text] (but preserve wikilinks in content)
    .replace(/\[\[([^\]]+)\]\](?!\()/g, '[$1]')
    // Remove empty brackets []
    .replace(/\[\]\s*/g, '')
    // Remove backslash line continuations (RTF artifact)
    .replace(/\\\n/g, '\n')
    .replace(/\\$/gm, '')
  ...
  // Fix newlines in reading time: (4 minute\nread) -> (4 minute read)
  cleaned = cleaned.replace(/\((\d+)\s*minute\s*\n\s*read\)/gi, '($1 minute read)');

  // Clean TLDR tracking URLs - extract actual destination
  // Pattern: https://tracking.tldrnewsletter.com/.../https:%2F%2Factual-url.com%2F.../tracking-suffix
  cleaned = cleaned.replace(/\[([^\]]+)\]\((https:\/\/tracking\.tldrnewsletter\.com\/[^)]+)\)/g, (match, text, url) => {
    // Find encoded destination URL
    const encodedMatch = url.match(/https:%2F%2F([^/]+)/);
    if (encodedMatch) {
      try {
        // Extract the path after the domain
        const urlStart = url.indexOf('https:%2F%2F');
        const remainder = url.slice(urlStart);
        // Find where tracking suffix starts (usually /1/ or /2/)
        const trackingMatch = remainder.match(/\/\d+\/[^/]+$/);
        const encodedUrl = trackingMatch ? remainder.slice(0, trackingMatch.index) : remainder;
...
/**
 * Result of saving to vault (includes Dropbox path for filing pipeline)
 */
...
/**
 * Update indexes for a newly created note
 * Updates both tag index (fast) and embeddings (async)
 * Uses dynamic import to avoid circular dependencies
 */
async function updateNoteIndexes(notePath: string): Promise<void> {
  // Update tag index first (fast, no API calls)
  try {
    const { updateNoteTagsInIndex } = await import("../../ctx/lib/tag-index");
    await updateNoteTagsInIndex(notePath);
  } catch {
    // Tag indexing is optional - silently fail
  }

  // Update embeddings (slower, requires API)
  try {
    const { embedNote } = await import("../../ctx/lib/embed");
...
export async function saveToVault(
  ...
  const isFilingPipeline = processed.pipeline === "filing";
  const isAttachPipeline = processed.pipeline === "attach";
  const subfolder = isFilingPipeline ? "filing" : "";
  ...
  // Attachment uses original filename (use /rename command for custom names)
  const attachmentFilename = originalFilename;

  // For attach pipeline: copy original file to vault/attachments/
  let attachmentPath: string | undefined;
  if (isAttachPipeline && processed.originalFilePath && attachmentFilename) {
    const attachmentsDir = join(config.vaultPath, "attachments");
    if (!existsSync(attachmentsDir)) {
      await mkdir(attachmentsDir, { recursive: true });
    }
    attachmentPath = join(attachmentsDir, attachmentFilename);
    ...
    console.log(`  Attached: ${attachmentFilename} → attachments/`);
  }

  // For filing pipeline: sync original file to Dropbox FIRST (so we can include link in markdown)
  let dropboxPath: string | undefined;
  if (isFilingPipeline && processed.originalFilePath && processed.archiveName) {
  ...
  const frontmatter = [
    ...
    ...(attachmentPath ? [`attachment: "attachments/${attachmentFilename}"`] : []),
    ...
    ...(meta?.dropFile ? [`drop_file: "${meta.dropFile}"`] : []),
    // Filing-specific metadata (only for filing pipeline)
    ...(processed.pipeline === "filing" && meta?.type ? [`document_type: ${meta.type}`] : []),
    ...(processed.pipeline === "filing" && meta?.category ? [`document_category: ${meta.category}`] : []),
    ...(processed.pipeline === "filing" && meta?.date ? [`document_date: ${meta.date}`] : []),
    "---",
  ].join("\n");
  ...
  if (attachmentPath && attachmentFilename) {
    const attachLink = `📎 **Attachment:** [[attachments/${attachmentFilename}]]\n\n`;
    content = attachLink + content;
  }

  // Add Dropbox link at top of content for filing items
  if (dropboxPath) {
  ...
  // Clean markdown artifacts before writing (Pandoc attributes, RTF escapes, etc.)
  content = cleanMarkdownArtifacts(content);

  await Bun.write(filePath, `${frontmatter}\n\n${content}`);

  // Update indexes for the new note (fire-and-forget, non-blocking)
  updateNoteIndexes(filePath).catch(() => {
    // Silently ignore indexing errors - search will work when batch rebuild runs
  });
