 ...
 * token-scope gap (found 2026-08-03: a narrowly-scoped CLOUDFLARE_API_TOKEN
 * threw on workers/routes 403, then storage/kv/namespaces 401, discarding
 * everything already gathered each time). One missing permission on one
 * resource type must degrade that signal only, never the whole collector —
 * matches Atlas's own doctrine ("a partial page, an API error → PARTIAL run,
 * no sweep, prior state intact"), which the bare cf() calls didn't actually
 * implement for this failure class. Sets `partial` via the caller-owned flag
 * so the run reports incomplete (never sweeps) instead of claiming success
 * over data it couldn't fully see.
 */
...
export const cloudflare: Collector = {
  ...
  // on most public installs. A source that EXISTS but is malformed still throws.
    ...
    // lacking "Workers Routes:Read" turned a single zone's routes lookup into a
    // total loss instead of a partial one.
    let partial = false;
