...
- `LoadContext.hook.ts` - Injects dynamic context (relationship, learning, work summary) as `<system-reminder>` at session start. Emits **two** work blocks: *Recent Sessions* (the push window, `isaPickup.recentWorkWindowHours`) and *Stalled ISAs* — non-terminal ISAs from the backlog index, most-recently-touched first, deduped against Recent and capped for display only. The index itself keeps every ISA forever; what this block *shows* is bounded by `isaPickup.stalledMaxAgeDays` (30 days by default, `0` for genuinely any age), and the heading states whichever window is in force. Both blocks share one terminal vocabulary (`TERMINAL_PHASES` in `hooks/lib/isa-index.ts`), so "done" cannot mean two things on one screen.
