...
| Reviewer runs | `MEMORY/OBSERVABILITY/reviewer-runs.jsonl` | — | `MemoryReviewer.ts` (autonomic memory). One row per reviewer execution: runId, transcript path, exchanges read, inference_duration_ms, parse_ok, dispatch_summary { total, by_type, succeeded, failed, failures, proposals_auto_applied, proposals_auto_apply_failed, and (7.40.4+) skipped_guard, skips }. `CortexHealth` accepts the pre-7.40.4 summary (missing skip pair = zero guard refusals) so an upgrade does not mark a successful last run CRITICAL. |
