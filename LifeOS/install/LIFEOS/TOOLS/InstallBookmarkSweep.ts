...
async function linuxSpec(): Promise<systemd.UnitSpec> {
  ...
  return {
    label: LABEL,
    description: "LifeOS bookmark sweep",
    exec: [bunPath, join(HOME, ".claude", "LIFEOS", "TOOLS", "BookmarkSweep.ts")],
    logPath: join(HOME, ".claude", "LIFEOS", "MEMORY", "STATE", "com.lifeos.bookmarksweep.log"),
...
async function linuxMain(arg: string | undefined): Promise<void> {
  ...
  const log = (m: string) => console.log(`[InstallBookmarkSweep] ${m}`);
