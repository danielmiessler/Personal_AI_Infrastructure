 ...
 * shape set by the WorkSweep systemd PR and by `PULSE/manage.sh`, which has
 * always branched `systemctl --user` on linux and `launchctl` on darwin.
 *
 ...
 *
 * The WorkSweep PR shipped a `.service.template` + `.timer.template` pair
 * beside its plist. That is right for one job. Across the eleven remaining
 * installers it would mean roughly twenty template files, all near-identical,
 * each able to drift from the installer that materializes it. So the unit text
 * is generated from a small typed spec instead, and each installer declares
 * only what is genuinely its own: its label, its command, its log file, and
 * its schedule. Adding the twelfth job is a ten-line branch, not two new files.
 *
...
export function renderService(spec: UnitSpec): string {
  ...
  for (const [k, v] of Object.entries(spec.environment ?? {})) lines.push(`Environment=${k}=${esc(v)}`);
  if (daemon) {
 ...
 *
 * Same class of bug as the `process.env.USER` issue a review caught on the
 * WorkSweep PR: an environment variable that happens to be set in a terminal
 * and absent everywhere else. Resolved here rather than left to the caller.
 */
 ...
 *  shell may not set USER, and an empty name makes enable-linger fail. This is
 *  the fix a review caught on the WorkSweep PR; inherited here deliberately. */
