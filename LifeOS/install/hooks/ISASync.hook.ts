...
async function main() {
  ...
  if (newPhase === 'COMPLETE' && oldPhase !== 'COMPLETE' && fm.slug) {
    try {
      const isaRender = join(process.env.LIFEOS_DIR || join(homedir(), '.claude', 'LIFEOS'), 'TOOLS/ISARender.ts');
  ...
  if (input.session_id) {
    try {
      const stateDir = join(process.env.LIFEOS_DIR || join(homedir(), '.claude', 'LIFEOS'), 'MEMORY/STATE/isa-render-debounce');
