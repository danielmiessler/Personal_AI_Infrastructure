...
## Integration with kai-security-tools

The `security-scan.yml` template is designed to integrate with the kai-security-tools pack. For enhanced scanning:

```yaml
include:
  - local: '/Templates/security-scan.yml'

# Override with kai-security-tools integration
secrets-scan:
  script:
    - kai-security scan secrets --format gitlab-ci
```
