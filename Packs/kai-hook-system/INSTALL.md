...
**IMPORTANT:** Use the full path to bun (`~/.bun/bin/bun`) instead of just `bun`, since Claude Code runs hooks in a minimal shell environment without your full PATH.
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.bun/bin/bun run $PAI_DIR/hooks/initialize-session.ts"
          },
          {
            "type": "command",
            "command": "~/.bun/bin/bun run $PAI_DIR/hooks/load-core-context.ts"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.bun/bin/bun run $PAI_DIR/hooks/security-validator.ts"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.bun/bin/bun run $PAI_DIR/hooks/update-tab-titles.ts"
          }
        ]
      }
    ]
...
If you see errors like `Stop hook error: Failed with non-blocking status code: /bin/sh: 1: bun: not found`:

**Cause:** Claude Code runs hooks in a minimal shell environment (`/bin/sh`) without your full PATH.

**Fix:** Use the full path to bun in `~/.claude/settings.json`:

```bash
# Find your bun installation path
which bun

# Update settings.json to use full path instead of just "bun"
# Change: "command": "bun run ..."
# To:     "command": "/home/username/.bun/bin/bun run ..."
# Or:     "command": "~/.bun/bin/bun run ..."
```
