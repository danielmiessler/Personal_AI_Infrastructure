...
export interface AdapterConfig {
  ...
  url?: string;
  /** Port for local services (e.g., Joplin) */
  port?: number;
  /** Authentication reference */
  auth?: AuthReference;
  /** Request timeout in milliseconds */
  timeout?: number;
  /** Default project/notebook for new issues */
...
/**
 * Get issues domain configuration
 */
export async function getIssuesConfig(): Promise<DomainConfig | undefined> {
  const config = await loadConfig();
  return config.domains?.issues;
}

/**
 * Get adapter configuration for issues domain
 */
export async function getAdapterConfig(adapterName: string): Promise<AdapterConfig> {
  const issuesConfig = await getIssuesConfig();

  if (!issuesConfig?.adapters) {
    return {};
  }

  return issuesConfig.adapters[adapterName] || {};
}
