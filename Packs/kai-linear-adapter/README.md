...
Linear project management adapter for KAI issues domain. This adapter implements the IssuesProvider interface from kai-issues-core using Linear's GraphQL API.
...
```bash
bun add kai-linear-adapter
```

## Usage

```typescript
import LinearAdapter from 'kai-linear-adapter';
