...
describe('ConfigLoader', () => {
  const testDir = path.join(os.tmpdir(), 'kai-network-core-test-' + Date.now());
...
describe('AdapterLoader', () => {
  const testDir = path.join(os.tmpdir(), 'kai-adapter-test-' + Date.now());
  ...
  test('loadManifest parses valid adapter.yaml', async () => {
    const adapterDir = path.join(testDir, 'kai-test-adapter');
...
`;
  ...
  test('loadManifest throws on invalid manifest', async () => {
    const adapterDir = path.join(testDir, 'kai-invalid-adapter');
    fs.mkdirSync(adapterDir, { recursive: true });
