...
```yaml
name: kai-standup-skill
version: 1.0.0
type: skill
requires:
  - kai-council-framework >= 1.0.0
  - kai-devsecops-agents >= 1.0.0
author: PAI
license: MIT
```
...
1. **Council Framework** - Core orchestration engine
   ```bash
   cd ~/PAI/Packs
   git clone <repo> kai-council-framework
   cd kai-council-framework && bun install
   ```

2. **DevSecOps Agents** - Agent pack with specialist personas
   ```bash
   cd ~/PAI/Packs
   git clone <repo> kai-devsecops-agents
   ```
...
```bash
cd ~/PAI/Packs
git clone <repo> kai-standup-skill
cd kai-standup-skill && bun install
```
...
- **joplin**: Saves to Joplin notebook (requires kai-joplin-skill)
...
// or
import { saveNote } from 'kai-joplin-skill/Tools/notes';
```
...
```typescript
import { runStandup } from 'kai-standup-skill/Workflows/RunStandup';
