...
export interface AdapterConfig {
  ...
  url?: string;
  /** API URL (alias for url) */
  apiUrl?: string;
  /** Host for GitLab instances */
  host?: string;
...
/**
 * Get CI/CD domain configuration
 */
export async function getCICDConfig(): Promise<DomainConfig | undefined> {
  const config = await loadConfig();
  return config.domains?.cicd;
}

/**
 * Get adapter configuration for CI/CD domain
 */
export async function getAdapterConfig(adapterName: string): Promise<AdapterConfig> {
  const cicdConfig = await getCICDConfig();

  if (!cicdConfig?.adapters) {
    return {};
  }

  return cicdConfig.adapters[adapterName] || {};
}
