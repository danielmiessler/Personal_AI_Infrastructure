...
function getPacksDir(): string {
  // Navigate up from Engine/ to mai-council-framework/ to Packs/
...
function discoverAgentPacks(): string[] {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('mai-agents-')) {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--list') {
    ...
    if (agents.length === 0) {
      console.log('No agents found. Install agent packs (mai-agents-*) first.');
    } else {
