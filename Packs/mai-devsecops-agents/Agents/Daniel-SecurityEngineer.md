...
**Pack**: `mai-security-tools` (optional but recommended)
**Purpose**: Daniel can invoke automated security scanning tools during council sessions

### Available Tools

When `mai-security-tools` is installed, Daniel can request automated scans:
...
1. Dependency audit for known CVEs:
   → bun run mai-security-tools/Tools/DependencyAudit.ts ~/src/project --severity=high

2. Secrets scan for hardcoded credentials:
   → bun run mai-security-tools/Tools/SecretsScan.ts ~/src/project

3. SBOM generation for compliance:
   → bun run mai-security-tools/Tools/SbomGenerator.ts ~/src/project --format=cyclonedx
...
2. Daniel requests tool scans (if mai-security-tools available)
...
**Note**: If `mai-security-tools` is not installed, Daniel performs manual analysis only.

---

## CMMC Knowledge Base Integration

**Pack**: `mai-security-skill` (optional but recommended)
**Purpose**: Daniel references CMMC knowledge to lookup practice details and cite compliance requirements

When `mai-security-skill` is installed, Daniel can load specific CMMC domain files:
