...
```yaml
name: mai-devsecops-agents
version: 1.0.0
type: agent-pack
requires: mai-council-framework >= 1.0.0
...
cp -r mai-devsecops-agents/* $PAI_DIR/skills/Council/agents/

# Or symlink for development
ln -s $(pwd)/mai-devsecops-agents/Agents $PAI_DIR/skills/Council/agents
```
...
### mai-security-tools (Recommended)

When `mai-security-tools` is installed, Daniel (Security Engineer) can leverage automated scanning:

```bash
# Daniel can request these scans during security reviews:
bun run mai-security-tools/Tools/DependencyAudit.ts [path]  # CVE scanning
bun run mai-security-tools/Tools/SecretsScan.ts [path]      # Secrets detection
bun run mai-security-tools/Tools/SbomGenerator.ts [path]    # SBOM generation
```
...
### mai-security-skill (Recommended)

When `mai-security-skill` is installed, Daniel can reference detailed CMMC knowledge:
