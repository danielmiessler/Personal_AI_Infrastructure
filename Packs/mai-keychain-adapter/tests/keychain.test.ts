...
import { SecretNotFoundError } from 'mai-secrets-core';

describe('KeychainAdapter', () => {
  const adapter = new KeychainAdapter({
    servicePrefix: 'mai-test',
  ...
  const testService = `mai-test:${testKey}`;
  ...
  describe('healthCheck', () => {
    it('returns healthy status on macOS', async () => {
      ...
      expect(status.details?.servicePrefix).toBe('mai-test');
    });
  });
...
describe('KeychainAdapter - SecretValue redaction', () => {
  const adapter = new KeychainAdapter({ servicePrefix: 'mai-test' });
