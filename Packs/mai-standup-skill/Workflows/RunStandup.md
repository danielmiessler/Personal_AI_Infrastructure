...
```typescript
import { DomainMapping } from 'mai-devsecops-agents/DomainMapping';
...
import { FileAdapter } from 'mai-council-framework/Adapters/FileAdapter';
  ...
  try {
    ...
    if (error instanceof AgentPackNotFoundError) {
      console.error(
        ...
        `Please install mai-devsecops-agents:\n` +
        `  cd ~/PAI/Packs && git clone <repo> mai-devsecops-agents\n`
      );
