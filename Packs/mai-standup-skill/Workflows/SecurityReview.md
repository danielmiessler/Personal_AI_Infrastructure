...
If `mai-security-tools` is installed, run automated scans before the council discussion:
...
async function runAutomatedSecurityScans(
  ...
  const toolsPath = `${PAI_DIR}/packs/mai-security-tools/Tools`;

  // Check if mai-security-tools is installed
  if (!existsSync(toolsPath)) {
    console.log('[mai-security-tools not installed - skipping automated scans]');
    console.log('[Install with: Install mai-security-tools pack]');
