...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    SERVICE_FILE="$SYSTEMD_DIR/pai-voice-server.service"
    LOG_DIR="$HOME/.local/share/logs"
    LOG_PATH="$LOG_DIR/pai-voice-server.log"
else
...
if [ "$OS_TYPE" = "Darwin" ]; then
    launchctl list | grep -q "$SERVICE_NAME" 2>/dev/null && ALREADY_INSTALLED=true
elif [ "$OS_TYPE" = "Linux" ]; then
    systemctl --user is-enabled pai-voice-server >/dev/null 2>&1 && ALREADY_INSTALLED=true
fi
...
if [ "$ALREADY_INSTALLED" = true ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [ "$OS_TYPE" = "Darwin" ]; then
            ...
            systemctl --user stop pai-voice-server 2>/dev/null || true
...
if [ -f "$ENV_FILE" ] && grep -q "ELEVENLABS_API_KEY=" "$ENV_FILE"; then
    ...
    if [ "$API_KEY" != "your_api_key_here" ] && [ -n "$API_KEY" ]; then
        ...
        echo "  Voice server will use system TTS command as fallback"
...
EOF
    ...
    echo -e "${GREEN}OK systemd user service created${NC}"

    # Reload and start the service
    echo -e "${YELLOW}> Starting voice server service...${NC}"
    systemctl --user daemon-reload
    systemctl --user enable pai-voice-server 2>/dev/null || {
        echo -e "${RED}X Failed to enable service${NC}"
        echo "  Try manually: systemctl --user enable pai-voice-server"
        exit 1
    }
    systemctl --user start pai-voice-server 2>/dev/null || {
        echo -e "${RED}X Failed to start service${NC}"
        echo "  Try manually: systemctl --user start pai-voice-server"
        exit 1
    }
fi
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    echo
    echo -e "${BLUE}Linux service management:${NC}"
