...
No tool calls except TaskCreate, voice notifications (pai-notify), and CONTEXT RECOVERY searches (see below) until the Quality Gate shows OPEN.
...
**OUTPUT 2 — 🎯 IDEAL STATE CRITERIA** (the ONLY tool calls in OBSERVE besides voice notifications, CONTEXT RECOVERY, and WISDOM INJECTION reads):
...
Interactive mode runs all phases (OBSERVE → THINK → PLAN → BUILD → EXECUTE → VERIFY → LEARN), creates ISC via TaskCreate, uses voice notifications (pai-notify), performs capability audits, and produces formatted output.
...
- No voice notifications (pai-notify) — only the parent orchestrator announces
...
Voice notifications serve dual purposes: (1) spoken phase announcements, and (2) dashboard phase-progression tracking. Skipping a curl breaks dashboard visibility into Algorithm execution, making it essential infrastructure — not optional audio.

Each curl is marked `[VERBATIM - Execute exactly as written, do not modify]` in the template. Execute each one as a Bash command when you reach that phase. Voice notifications are the ONLY Bash commands allowed in OBSERVE (before the Quality Gate opens).
...
- Each gets its own voice notification announcement (MANDATORY — see Voice Phase Announcements)
...
!!! NOTE: Background agents don't need to execute the voice notifications!!! They are annoying to hear and distracting. Only the main agent is supposed to be executing the mandatory voice notification commands!
...
- **No tool calls in OBSERVE** except TaskCreate, voice notifications, and CONTEXT RECOVERY (Grep/Glob/Read on memory stores only, ≤34s total). Reading code before ISC exists = premature execution. Reading your own prior work notes = understanding the problem.
