...
📋 **DEPLOY TARGET CHECK (v1.8.1):** If the project has a known deploy target (in MEMORY, CLAUDE.md, or project config), auto-include `ISC-DEPLOY: Service running on {target} with committed changes` in the criteria. "Commit and push" for a project with a production server means "make it live" — deploy verification is not optional.
...
📋 **OUTPUT TEMPLATE (v1.8.1):** For tasks with structured output (reports, analyses, reviews), write the output skeleton with placeholder sections in PLAN before dispatching research agents. EXECUTE becomes fill-in-the-blanks — don't waste context on structural decisions during synthesis.
...
[Create artifacts]
🔧 **BATCH FILE EDITS (v1.8.1):** When BUILD involves 4+ file edits, read all target files in parallel first, plan all edits, then apply edits in parallel. Interleaving reads and edits causes unnecessary round-trips.
