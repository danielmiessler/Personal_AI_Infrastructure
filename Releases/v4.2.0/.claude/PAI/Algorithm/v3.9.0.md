...
### Voice Announcements (Optional — Disabled by Default)

Voice is controlled by `voice.enabled` in `settings.json`. When disabled (default), skip all voice announcements — use console output only.

When enabled, announce at Algorithm entry and every phase transition via fire-and-forget fetch to VoiceServer.
...
**CRITICAL: Only the primary agent may execute voice announcements.** Background agents, subagents, and teammates spawned via the Task tool must NEVER make voice calls. Voice is exclusively for the main conversation agent.
