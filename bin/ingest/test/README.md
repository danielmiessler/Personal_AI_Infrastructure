...
bun run ingest.ts test run --skip-media  # Skip media for faster runs
...
Some tests produce complex, non-deterministic outputs that can't be validated with simple string matching. These tests use **LLM-as-judge** - Claude automatically evaluates the semantic quality of outputs.

### Running Tests

LLM-as-judge runs **by default** for tests with semantic validation specs:
...
# Results saved to: test/output/<run-id>/semantic-validation.json
```
...
| Test ID | Why Semantic Validation? |
|---------|-------------------------|
| TEST-PAT-001 | Fabric `meeting_minutes` pattern output |
| TEST-PAT-002 | Fabric `summarize` pattern output |
| TEST-PAT-003 | Fabric `extract_wisdom` pattern output |
| TEST-REG-003 | Mixed metadata extraction quality |
| TEST-REG-004 | Jina AI URL extraction + wisdom |
| TEST-REG-007 | Vision AI image description |
| TEST-REG-011 | Mermaid diagram extraction |
| TEST-REG-020 | iOS clipboard content processing |
| TEST-PHOTO-001 | Vision AI `/describe` output |

### When to Add Semantic Validation

Add semantic validation when:
...
Don't add when:
- ❌ Output is **deterministic** (tag extraction, date parsing)
- ❌ Simple **keyword/structure checks** are sufficient
- ❌ Test validates **pipeline routing** not content quality

### Adding Semantic Validation to a Test

Add the `semantic` field to the test spec's `expected` section:
...
```typescript
{
  ...
  expected: {
    ...
    semantic: {
      description: "What the output should achieve",
      checkpoints: [
        "Specific thing to verify #1",
        "Specific thing to verify #2",
      ],
      target: "wisdom",  // "raw" | "wisdom" - which file to evaluate
      threshold: 80,     // Minimum confidence % for pass
    },
  },
}
```
...
Media tests (voice, photo, document) **run by default** because they're core pipeline functionality. Use `--skip-media` for faster runs during development:
...
**Why media tests run by default:**
- Voice transcription, Vision AI, and document processing are critical features
- Skipping them risks undetected regressions
- The `--skip-media` flag is an opt-out for speed, not the default
