...
/**
 * Sync a single file to knowledge graph
 */
async function syncFile(
  ...
  try {
    ...
    if (result.success) {
      if (options.verbose) {
        console.error(`[Sync] ✓ Synced: ${params.name}`);
      }
      return { success: true };
    } else {
      console.error(`[Sync] ✗ Failed: ${params.name} - ${result.error}`);
...
async function syncHistoryToKnowledge(options: SyncOptions = DEFAULT_OPTIONS): Promise<{
  ...
  if (!options.dryRun) {
    ...
    if (!healthy) {
      console.error('[Sync] MCP server offline - skipping sync');
      return stats;
    }
  }
